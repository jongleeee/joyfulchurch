...
@interface AWSAPI_LOGQZC12CB_CreateSermonMobileHubClient()
...
@implementation AWSAPI_LOGQZC12CB_CreateSermonMobileHubClient

static NSString *const AWSInfoClientKey = @"AWSAPI_LOGQZC12CB_CreateSermonMobileHubClient";
...
+ (instancetype)defaultClient {
    ...
    static AWSAPI_LOGQZC12CB_CreateSermonMobileHubClient *_defaultClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _defaultClient = [[AWSAPI_LOGQZC12CB_CreateSermonMobileHubClient alloc] initWithConfiguration:serviceConfiguration];
    });
...
+ (void)registerClientWithConfiguration:(AWSServiceConfiguration *)configuration forKey:(NSString *)key {
    ...
    [_serviceClients setObject:[[AWSAPI_LOGQZC12CB_CreateSermonMobileHubClient alloc] initWithConfiguration:configuration]
                        forKey:key];
}

+ (instancetype)clientForKey:(NSString *)key {
    @synchronized(self) {
        AWSAPI_LOGQZC12CB_CreateSermonMobileHubClient *serviceClient = [_serviceClients objectForKey:key];
        if (serviceClient) {
...
- (instancetype)initWithConfiguration:(AWSServiceConfiguration *)configuration {
    if (self = [super init]) {
        ...
        NSString *URLString = @"https://logqzc12cb.execute-api.us-west-1.amazonaws.com/Development";
