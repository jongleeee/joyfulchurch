...
- (void)loadAppearance {
    ...
    [settings loadSettings:^(ColorThemeSettings *themeSettings, NSError *error) {
        if (error) {
            AWSLogError("Failed to load color from dataset: %@", error);
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self updateAppearanceWithTheme:themeSettings.theme];
            });
        }
    }];
}
...
- (void)updateAppearanceWithTheme:(Theme *)theme {
    ...
    [self.navigationController.viewControllers firstObject].view.backgroundColor = UIColorFromARGB(theme.backgroundColor);
